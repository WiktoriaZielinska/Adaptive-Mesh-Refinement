...
### Step 17: Let's create a new run.sh, type: ```vim runAdaptiveMeshRefinement.sh```
...
### Step 21: Before saving, play around with this line:

```
parthenon/mesh/numlevel=5 \
```

### Step 22: What happens if you change the ```5``` to a ```3```, or some other number? Try changing it to ```3```, or whatever number you decide on. Use the arrow keys to move around:
...
### Step 23: Press ```Esc``` on the keyboard:
...
### Step 25: Press ```Enter``` on the keyboard:

![image](https://github.com/user-attachments/assets/296be11f-d36d-434f-85b1-f037ddeed497)

### Step 26: To confirm that your file is there, type: ```ls```
...
### Step 31: Check the status of your job again. No time means that your job is done:
...
### Step 32: Enter the ```ls``` command and press ```Enter``` to see all of the new files that have been created:

![image](https://github.com/user-attachments/assets/efa72c19-61d8-459e-83ac-a06a5b8f2474)

### Step 33: It is now time to visualize our files in VisIt. Open up the two VisIt windows:
